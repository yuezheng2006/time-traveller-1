...
declare module 'motia' {
  ...
  interface Handlers {
    'SynthesizeSpeech': EventHandler<{ teleportId: string; text: string; language: string }, never>
    'GenerateLocationDetails': EventHandler<{ teleportId: string; destination: string; era: string; language: string }, { topic: 'location-details-generated'; data: { teleportId: string } }>
    ...
    'UploadImage': ApiRouteHandler<{ image: string }, ApiResponse<201, { url: string }> | ApiResponse<400, { error: string }> | ApiResponse<401, { error: string }> | ApiResponse<500, { error: string }>, never>
    'SaveKeys': ApiRouteHandler<{ geminiKey?: string; mapsKey?: string }, ApiResponse<200, { success: boolean }> | ApiResponse<401, { error: string }>, never>
    'ParseTravelCommand': ApiRouteHandler<{ message: string; history: Array<string>; language: string }, ApiResponse<200, { isJump: boolean; reply: string; params?: { destination: string; era: string; style: string; aspectRatio?: string; imageSize?: string } }> | ApiResponse<400, { error: string }>, never>
    'InitiateTeleport': ApiRouteHandler<{ destination: string; era: string; style: string; referenceImage?: string; referenceImages?: Array<{ id: string; data: string; type: 'person' | 'celebrity' | 'object'; label?: string }>; coordinates?: { lat: number; lng: number }; imageConfig?: { aspectRatio: '1:1' | '2:3' | '3:2' | '3:4' | '4:3' | '4:5' | '5:4' | '9:16' | '16:9' | '21:9'; imageSize: '1K' | '2K' | '4K' }; userGeminiKey?: string; userMapsKey?: string; language: string }, ApiResponse<201, { teleportId: string; status: string; message: string }> | ApiResponse<400, { error: string }> | ApiResponse<401, { error: string }>, { topic: 'generate-image'; data: { teleportId: string; destination: string; era: string; style: string; coordinates?: { lat: number; lng: number }; referenceImages?: Array<{ id: string; data: string; type: 'person' | 'celebrity' | 'object'; label?: string }>; imageConfig?: { aspectRatio: '1:1' | '2:3' | '3:2' | '3:4' | '4:3' | '4:5' | '5:4' | '9:16' | '16:9' | '21:9'; imageSize: '1K' | '2K' | '4K' }; userGeminiKey?: string; userMapsKey?: string } } | { topic: 'generate-location-details'; data: { teleportId: string; destination: string; era: string; language: string } }>
    'GetUser': ApiRouteHandler<Record<string, unknown>, ApiResponse<200, { user: { id: string; email: string; name: string; avatarUrl?: string; isWhitelisted?: boolean } }> | ApiResponse<401, { error: string }> | ApiResponse<404, { error: string }> | ApiResponse<500, { error: string }>, never>
    ...
    'Auth': ApiRouteHandler<{ authToken: string }, ApiResponse<200, { accessToken: string; user: { id: string; email: string; name: string; avatarUrl?: string; isWhitelisted?: boolean } }> | ApiResponse<401, { error: string }> | ApiResponse<500, { error: string }>, never>
  }
