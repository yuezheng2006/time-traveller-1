...
export const HistoryLog: React.FC<HistoryLogProps> = ({ history, onSelect, currentId }) => {
  ...
  return (
    <div className="bg-cyber-800/80 border border-cyber-700 rounded-xl p-3 lg:p-4 shadow-[0_0_30px_rgba(0,0,0,0.3)] flex flex-col h-full max-h-full backdrop-blur-md relative overflow-hidden">
      {/* Decorative header line */}
      <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-cyber-500/50 to-transparent"></div>
      
      <h3 className="text-xs font-bold text-cyber-500 mb-3 font-mono uppercase tracking-[0.2em] flex items-center gap-2">
      ...
      <div className="flex-1 overflow-y-auto space-y-1.5 pr-1 scrollbar-thin min-h-0">
        {history.map((item, index) => (
          <button
            ...
            className={`w-full text-left p-2.5 rounded-lg border transition-all duration-300 group relative overflow-hidden animate-[slideIn_0.3s_ease-out_both] ${
              currentId === item.id
          ...
          >
            ...
            <div className="flex gap-2.5">
                {/* Mini Thumbnail */}
                <div className={`w-11 h-11 rounded border overflow-hidden shrink-0 ${currentId === item.id ? 'border-cyber-500' : 'border-cyber-700 group-hover:border-cyber-500/50'}`}>
                    {item.imageData ? (
            ...
            <div className="flex justify-between items-end relative z-10">
                        <div className="flex flex-row items-center gap-2 mt-0.5">
                            <span className="text-[10px] text-slate-400 font-mono flex items-center gap-1 truncate max-w-[100px]">
                                <Clock className="w-2.5 h-2.5 opacity-50" /> {item.era}
                            </span>
                            <span className="text-[9px] text-cyber-600 uppercase tracking-wider truncate border-l border-cyber-900/40 pl-2">{getStyleLabel(item.style, t)}</span>
                        </div>
